...
| A-LOBBY-04 | Station coordinates and prompt range | Measured from the owner's lobby map: the barn doorway is the portal, the UPGRADES stall counter is the perk shop, the CLASSES stall counter is the class station, the middle leaderboard board is Stats, the Daily Nest and Moonlit Chest sit on the barn floor in front of the nesting shelf, the right coop is the Tool Rack and the left coop the Nest Mailbox; 10-stud prompts | Every station lives on a prop the map already provides so nothing reads as dropped on top. | `ChapterLayout.lobby.stations` (presentation), `lobby.stationPromptRangeStuds` |
...
| A-LOBBY-06 | Party nest pads | Standing inside one of the four "Join Match" nest pads joins that pad's party (created on first entry, default Chapter 1 Normal); stepping off leaves it; a full pad sets off by itself after 5 s; the leader can set off early from the sign post | `lobby.padFullAutoStartSeconds`, `lobby.padZoneHeightStuds`, `ChapterLayout.lobby.pads` |
| A-UX-09 | Map-fitted nest | Logical cells are 1.8 studs (16 x 16 = 28.8 studs) and the mound apex is 16 studs so the feather mountain fills the map's earth pit (32 x 48 opening, floor 5.6 studs below ground) and rises about 11 studs above the yard | `pile.cellSizeStuds`, `pile.maxVisualHeightStuds` |
